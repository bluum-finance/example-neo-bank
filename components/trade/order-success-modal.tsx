...
interface OrderSuccessModalProps {
  ...
  onPlaceAnother: () => void;
  side: Side;
  asset: { symbol: string; name: string; currency?: CurrencyCode };
  orderType: OrderType;
  shares: string;
...
export function OrderSuccessModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-6 p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        <div className="flex flex-col items-center justify-center gap-5 px-8 pt-10 pb-8 border-b border-[#1E3D2F]">
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            <p className="text-[#8DA69B] text-base font-normal font-inter leading-6 max-w-104.5">
              Your {orderType} {side} order for <span className="text-white font-medium">{asset.symbol}</span> has been successfully queued.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6 p-8">
          ...
          <div className="grid grid-cols-2 gap-y-8">
            <div className="flex flex-col gap-1">
              <span className="text-[#8DA69B] text-xs font-normal font-inter uppercase tracking-wider">Order ID</span>
              <span className="text-white text-sm font-normal font-inter">{orderId}</span>
            </div>
            <div className="flex flex-col gap-1 items-end">
              <span className="text-[#8DA69B] text-xs font-normal font-inter uppercase tracking-wider">Status</span>
              <span className="px-2 py-0.5 bg-[#1A3A2C] rounded-lg text-xs font-medium font-inter text-[#93C5FD]">Pending Fill</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[#8DA69B] text-xs font-normal font-inter uppercase tracking-wider">Estimated Cost</span>
            ...
            <div className="flex flex-col gap-1 items-end">
              <span className="text-[#8DA69B] text-xs font-normal font-inter uppercase tracking-wider">Shares</span>
              <span className="text-white text-lg font-semibold font-inter leading-7">{parseFloat(shares).toLocaleString()}</span>
            </div>
          </div>
      ...
      <p className="max-w-96 text-center text-xs font-normal font-inter text-[#9CA3AF] leading-4">
        Market orders are executed at the best available price. Settlement typically occurs within T+2 business days.
      </p>
    </div>
  );
}
