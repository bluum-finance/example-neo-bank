...
import { getDemoMarketMoverSymbols, getDemoMarketMovers } from '@/lib/demo/assets';

const { gainers: SEED_GAINERS, losers: SEED_LOSERS } = getDemoMarketMovers();
const MOVER_SYMBOLS = getDemoMarketMoverSymbols();
  ...
  return (
    <Link href={assetDetailHref(mover.symbol, mover.market)} className="group flex items-center justify-between py-3 first:pt-0 last:pb-0 transition-opacity hover:opacity-90 cursor-pointer">
      <div className="flex items-center gap-3">
        <div className="overflow-hidden flex h-8 w-8 shrink-0 items-center justify-center rounded bg-[#1E3D2F] font-manrope text-[8px] font-bold text-white uppercase tracking-tighter">
          {mover.symbol}
        </div>
...
export function MarketMoversOverview() {
  ...
  useEffect(() => {
    const fetchQuotes = async () => {
      try {
        ...
        setQuotes([...SEED_GAINERS, ...SEED_LOSERS]);
      } finally {
        setLoading(false);
      }
    };
  ...
  const { gainers, losers } = useMemo(() => {
    ...
    if (quotes.length > 0) {
      ...
      const liveGainers = SEED_GAINERS.map((g) => quoteMap.get(g.symbol) || g);
      const liveLosers = SEED_LOSERS.map((l) => quoteMap.get(l.symbol) || l);
      return { gainers: liveGainers, losers: liveLosers };
    }
    return { gainers: SEED_GAINERS, losers: SEED_LOSERS };
  }, [quotes]);
