...
export function InvestmentPolicyWidget({ policy }: InvestmentPolicyWidgetProps) {
  ...
  const timeHorizonDisplay =
    timeHorizonYears !== undefined ? `${timeHorizonYears} ${timeHorizonYears === 1 ? 'Year' : 'Years'}` : undefined;
  ...
  const timeHorizonProgress = timeHorizonYears !== undefined ? Math.min((timeHorizonYears / maxDisplayYears) * 100, 100) : undefined;
  ...
  const liquidityDescription =
    liquidityPercent !== undefined ? `Maintain ${liquidityPercent}% in cash/equivalents for operational needs.` : undefined;
  ...
  const restrictions = (() => {
    ...
    if (restr.esg_screening) {
      if (restr.esg_criteria && Array.isArray(restr.esg_criteria) && restr.esg_criteria.length > 0) {
        const criteria = restr.esg_criteria
    ...
    <div className="w-full flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5">
        ...
        <div className="p-4 rounded-xl bg-card border border-gray-200 dark:border-border flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-xs font-medium text-muted-foreground dark:text-[#D1D5DB]">Risk Tolerance</div>
          </div>
            ...
            <>
              <div className="flex flex-col gap-1">
                ...
                <div className="flex justify-between items-center w-full">
                  <span className="text-[10px] leading-[13.5px] font-normal text-gray-500 dark:text-muted-foreground">Conservative</span>
                  <span className="text-[10px] leading-[13.5px] font-normal text-gray-500 dark:text-muted-foreground">Moderate</span>
                  <span className="text-[10px] leading-[13.5px] font-normal text-gray-500 dark:text-muted-foreground">Aggressive</span>
                </div>
              </div>
              <div className="flex flex-col items-center gap-0">
                {riskLevel && <div className="w-full text-center text-base font-semibold leading-6 text-[#57B75C]">{riskLevel}</div>}
        ...
        <div className="p-4 rounded-xl bg-card border border-gray-200 dark:border-border flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-xs font-medium text-muted-foreground dark:text-[#D1D5DB]">Time Horizon</div>
          </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="w-full h-2 dark:bg-[#2A4D3C] bg-gray-200 rounded-full overflow-hidden">
                  {timeHorizonProgress !== undefined && (
                    <div className="h-full bg-[#57B75C] rounded-l-full" style={{ width: `${timeHorizonProgress}%` }} />
                  )}
                </div>
                {/* Time labels */}
                <div className="flex justify-between items-center w-full">
        ...
        <div className="p-4 rounded-xl bg-card border border-gray-200 dark:border-border flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-xs font-medium text-muted-foreground dark:text-[#D1D5DB]">Objectives</div>
          </div>
          ...
          {objectives.length > 0 ? (
            <div className="flex flex-col gap-1.5">
              {objectives.map((obj: { text: string; tag: 'PRIMARY' | 'SECONDARY' | 'TERTIARY' }) => {
                ...
                return (
                  <div key={obj.text} className="py-2 rounded-md flex justify-between items-center">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-normal text-muted-foreground dark:text-white/80">{obj.text}</span>
                    </div>
                    <div className={`h-4 px-1.25 py-0.75 rounded-[3px] flex items-center justify-center shrink-0 ${tagStyle}`}>
                      <span className="text-[8px] leading-3 font-semibold uppercase">{obj.tag.toLowerCase()}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="p-4 rounded-xl bg-card border border-gray-200 dark:border-border flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-xs font-medium text-muted-foreground dark:text-[#D1D5DB]">Target Allocation</div>
          </div>
          ...
          {allocationData.length > 0 ? (
            <>
              ...
              <div className="flex items-center gap-3.5">
                ...
                <div className="flex-1 flex flex-col gap-2">
                  {allocationData.map((item, index) => (
                    <div key={item.name} className="relative h-[15px]">
                      <div className="absolute left-0 top-[3.5px] w-2 h-2 rounded-xs" style={{ backgroundColor: item.color }} />
                ...
                <div className="flex flex-col gap-1 items-end shrink-0">
                  {allocationData.map((item) => (
                    <div key={item.name} className="text-[10px] leading-[15px] font-semibold text-gray-500 dark:text-muted-foreground">
                      {item.value}%
                    </div>
                  ))}
                </div>
              ...
              <div className="relative mt-1 flex items-center gap-2 px-2 py-1.5 w-full h-full rounded-[6px] bg-[rgba(48,209,88,0.12)]">
                ...
                <div className="text-[11px] leading-[15px] font-light text-[#30D158]">Within ±3% of target allocation</div>
              </div>
            </>
          ) : (
            <NoDataAvailable />
      ...
      <div className="p-4 rounded-xl bg-card border border-gray-200 dark:border-border flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="text-xs font-medium text-muted-foreground dark:text-[#D1D5DB]">Constraints & Guidelines</div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {liquidityDescription && (
              <div className="p-3 rounded-lg border border-gray-200 dark:border-border flex items-center gap-3">
                ...
                <div className="flex-1 flex flex-col gap-0.5">
                  <div className="text-[13px] font-medium text-gray-900 dark:text-foreground">Liquidity</div>
                  <div className="text-xs leading-[18px] text-gray-600 dark:text-muted-foreground">{liquidityDescription}</div>
                </div>
              </div>
            )}
            ...
            {taxConsiderations && (
              <div className="p-3 rounded-lg border border-gray-200 dark:border-border flex items-center gap-3">
                ...
                <div className="flex-1 flex flex-col gap-0.5">
                  <div className="text-[13px] font-semibold text-gray-900 dark:text-foreground">Tax Considerations</div>
                  <div className="text-xs leading-[18px] text-gray-600 dark:text-muted-foreground">{taxConsiderations}</div>
                </div>
              </div>
            )}
            ...
            {restrictions && (
              <div className="p-3 rounded-lg border border-gray-200 dark:border-border flex items-center gap-3">
                ...
                <div className="flex-1 flex flex-col gap-0.5">
                  <div className="text-[13px] font-semibold text-gray-900 dark:text-foreground">Restrictions</div>
                  <div className="text-xs leading-[18px] text-gray-600 dark:text-muted-foreground">{restrictions}</div>
                </div>
              </div>
            )}
            ...
            {rebalancing && (
              <div className="p-3 rounded-lg border border-gray-200 dark:border-border flex items-center gap-3">
                ...
                <div className="flex-1 flex flex-col gap-0.5">
                  <div className="text-[13px] font-semibold text-gray-900 dark:text-foreground">Rebalancing</div>
                  <div className="text-xs leading-[18px] text-gray-600 dark:text-muted-foreground">{rebalancing}</div>
                </div>
              </div>
            )}
          </div>
...
const NoDataAvailable = () => {
  return (
    <div className="flex flex-col items-center gap-0 py-2">
      <div className="w-full text-sm font-normal text-gray-500 dark:text-muted-foreground">No data available</div>
    </div>
  );
};
